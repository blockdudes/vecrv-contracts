...
module.exports = async function (deployer, network, accounts) {
  ...
  const booster = await Booster.at(boosterAdd);
  const pickle = await IERC20.at(contractList.system.pickle_address);
  logTransaction(
  ...
  logTransaction(
    await p3crv.transfer(admin, web3.utils.toWei("500"), { from: "0x1fe5F397e38fFe61E663d96821F41bCF83ed7959" }),
    "funcd account[0] with lp token p3CRV"
  );

  // for claim rewards
  // fund booster pickle token
  logTransaction(
    await pickle.transfer(booster.address, web3.utils.toWei("500"), { from: admin }),
    "fund pickle to booster"
  );
  logTransaction(await booster.earmarkRewards(0), "booster earmarkRewards");
  ...
  logTransaction(await booster.earmarkFees(), "booster earmarkFees");
};
